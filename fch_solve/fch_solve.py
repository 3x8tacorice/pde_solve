...
def CH_conserved_scheam(solmat):
    ...
    #for n in np.arange(10):
        ...
        x = optimize.root(DCH_eq,[0]*solmat.sp_size,
            args=(
                solmat.mat[:,n],
                solmat.h,
                solmat.dt,
...
def DCH_eq(x,u,dx,dt,D_2,alpha,period,frac=True):
    p = -1.0
    q = -0.0005
    r = 1.0
    u_half = (x+u)/2.
    DV = (p)*u_half + (r)*((x**3)+(x**2)*(u)+(x)*(u**2)+(u**3))/4. + (q)*np.dot(D_2,u_half)*(1./dx**2)
    if frac :
        f = (x - u) + f_diff.FFT_diff(DV,alpha,period).real*dt
    else:
