...
class Solmat:
    ...
    def __init__(self,alpha,sp_size,time_step,left,right,Tmax,beta):
        ...
        self.alpha = alpha
        self.beta = beta
        self.mat = np.zeros((self.sp_size,self.time_step),dtype=complex)
    ...
    def plot_Mass(self):
        ...
        plt.plot(self.ts[1:-1],err_Q/abs(self.Q_vec[1:]))
        plt.show()
    ...
    def plot_Animation(self,rate):
        fig = plt.figure(figsize=(10,8))

        rate = rate


        frames = np.array([plt.plot(self.xs, np.abs(self.mat[:,i*rate]),color="blue") for i in range(self.time_step/rate)])


        ani = animation.ArtistAnimation(fig, frames, interval=1)
        plt.show()
...
if __name__ == '__main__':
    solmat = Solmat(1.4,400,1000,-20,20,100,2)
    print solmat.xs
    print solmat.Tmax
